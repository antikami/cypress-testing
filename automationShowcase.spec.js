...
describe('First Test Suite', () => {
     ...
     it('Forms Dropdown Check', () =>{
       ...
       cy.get('#forms').then( forms => {
           ...
           cy.get('#homeSubmenu li').each((listItem, index ) => {
            ...
            cy.wrap(forms).click()
            cy.wait(1000)
            cy.wrap(listItem).should('not.be.visible')
           })
    ...
    it('Alerts', () => {
        ...
        cy.get('#alerts').click() 
        //cy.on('window:alert', (alert) => {    // simple alert window

         //   expect(alert).to.equal('Hello! I am an alert box!!')
